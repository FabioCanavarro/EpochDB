...
#[async_recursion]
pub async fn handle_response(res: Result<Response, TransientError>) -> Result<(), TransientError> {
    match res {
        Ok(r) => {
            match r {
...
#[async_recursion]
pub async fn parse_server_response<T: AsyncReadExt + Unpin + AsyncBufReadExt + Send>(
    stream: &mut T,
    buf: &mut Vec<u8>
...
pub async fn tcp_logic(cli: Cli, mut client: Client) -> Result<Response, TransientError> {
